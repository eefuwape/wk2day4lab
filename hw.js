...
function countYourBills(x, y) {
  ...
  let fiveBillCount = 0,
    twoBillCount = 0,
    OneDollarCount = 0;
    let one = '', two = '', five = '', ten = '', twenty = '', fifty = '', hundred = '';
  ...
  if (change > 0) {
    while (change > 0) {
      ...
      if (OneDollarCount !== 0) {
        one = `${OneDollarCount} $1 Bill(s) `
      } 
      if (twoBillCount !== 0) {
        two = `${twoBillCount} $2 Bill(s) `
      } 
      if (fiveBillCount !== 0) {
        five = `${fiveBillCount} $5 Bill(s) `
      } 
      if (tenBillCount !== 0) {
        ten = `${tenBillCount} $10 Bill(s) `
      } 
      if (twentyBillCount !== 0) {
        twenty = `${twentyBillCount} $20 Bill(s) `
      } 
      if (fiftyBillCount !== 0) {
        fifty = `${fiftyBillCount} $50 Bill(s) `
      } 
      if (hundredBillCount !== 0) {
        hundred = `${hundredBillCount} $100 Bill(s) `
      } 
    }
    let totalBillCount =
      hundredBillCount +
    ...
    console.log(
      `Your change of $${z} will result in a total of ${totalBillCount} bills. You will recieve ` + hundred + fifty + twenty + ten + five + two + one);
  } else {
